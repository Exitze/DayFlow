...
public enum ReleaseReadinessItemID: String, Equatable, CaseIterable {
...
public struct ReleaseReadinessItem: Equatable, Identifiable {
    public var id: ReleaseReadinessItemID
    public var title: String
    public var detail: String
    public var isHandledInProject: Bool

    public init(id: ReleaseReadinessItemID, title: String, detail: String, isHandledInProject: Bool) {
        self.id = id
        self.title = title
        self.detail = detail
        self.isHandledInProject = isHandledInProject
    }
}

public enum DayflowReleaseReadiness {
    public static let items: [ReleaseReadinessItem] = [
        ReleaseReadinessItem(
            id: .privacyManifest,
            title: "Privacy manifest",
            detail: "Добавлен PrivacyInfo.xcprivacy: нет трекинга, нет собираемых данных, UserDefaults указан с reason CA92.1.",
            isHandledInProject: true
        ),
        ReleaseReadinessItem(
            id: .localDataDeletion,
            title: "Удаление локальных данных",
            detail: "В настройках есть очистка выполненных дел, календарных деталей и полный сброс данных.",
            isHandledInProject: true
        ),
        ReleaseReadinessItem(
            id: .noRuntimePermissions,
        ...
        ReleaseReadinessItem(
            ...
            detail: "Нужно опубликовать privacy policy на публичной странице и вставить ссылку в App Store Connect.",
            isHandledInProject: false
        ),
        ReleaseReadinessItem(
            id: .supportURL,
            title: "Support URL",
            detail: "Нужна публичная страница поддержки или лендинг с контактом для пользователей.",
            isHandledInProject: false
        ),
        ReleaseReadinessItem(
            id: .appStoreMetadata,
            title: "Метаданные",
            detail: "Название, подзаголовок, описание, ключевые слова, категория, возрастной рейтинг и данные privacy label заполняются в App Store Connect.",
            isHandledInProject: false
        ),
        ReleaseReadinessItem(
            id: .screenshots,
            title: "Скриншоты",
            detail: "Нужно снять чистые скриншоты для нужных размеров iPhone и проверить, что интерфейс не содержит временных текстов.",
            isHandledInProject: false
        ),
        ReleaseReadinessItem(
            id: .testFlight,
            title: "TestFlight",
            detail: "Перед review стоит прогнать сборку через TestFlight на реальном устройстве.",
            isHandledInProject: false
        )
    ]

    public static var externalItems: [ReleaseReadinessItem] {
        items.filter { !$0.isHandledInProject }
    }
}
